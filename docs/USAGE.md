...
Retrostation 运行在双屏设备上：**上屏**显示游戏画面，**下屏**显示菜单与信息。下面是所有内置界面的实机截图。
...
当系统检测到多于一张卡时，设置菜单会多出 **「存储卡」** 一行：

![存储卡菜单](../screenshots/09-card-menu_top.png)

按 A 在 `TF1` / `TF2` 之间切换，会重启 Retrostation 并加载对应卡上的 ROM。单卡环境下这一行不会出现（可与上图 05 的设置菜单对照）。
