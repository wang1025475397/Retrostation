...
封面 / Logo 会按**每个显示尺寸**预先缩放好，存在源图所在目录的 `.cache/` 文件夹里（点开头，扫描器不会把它当成 ROM 或素材）。掌机的 CPU 太弱，滚动列表时现解一张全尺寸封面要几十毫秒，缓存是把首帧压到 1.2 s 的关键。

代价是卡空间：**每个「源图 × 尺寸」一个文件**，一个游戏通常 2~8 份，约 100~400 KB。

设置菜单（按 START）底部有两行管它：

- **「缩略图缓存」** —— 按 A 开 / 关。关掉后不再读写任何缓存文件，图片改为实时缩放。**桌面端几乎无感**（实测每图 5~10 ms），掌机上会明显变慢，建议保持开启。已生成的缓存不会被删除，重新打开即可继续用。
- **「清空图片缓存」** —— 按 A 删除卡上**所有** `.cache` 目录，回收空间。清完弹提示告知删了多少个文件；之后浏览时按需重建。
