...
lives at ``retrostation.platform.fonts`` -- *not* under ``platform.linux`` -- so
the Android and desktop adapters can reuse :class:`FontBook` without importing the
Linux package (``platform.linux.__init__`` pulls in SDL2 + evdev off the handheld).
"""
...
_CANDIDATE_DIRS: tuple[str, ...] = (
    ...
    "/system/fonts",                             # Android built-ins (Noto CJK/Roboto)
    "/product/fonts",                            # Android 12+ vendor fonts
...
_CANDIDATE_NAMES: tuple[str, ...] = (
    ...
    "simhei.ttf",
    "DejaVuSans.ttf",
    "Roboto-Regular.ttf",                        # Android Latin
    "PingFang.ttc",
)
